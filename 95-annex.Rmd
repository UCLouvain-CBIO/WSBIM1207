...
5. Click the `Environments` tab and the `New` button to create a
   ...
   pre-installed packages. The building will about 50 minutes. You can
   safely shut your computer down at this stage, as the environment is
   build in the cloud.
...
environment in a matter of seconds and connect.


It is easey to upload files directly to the remote RStudio instance by
using the `Upload` button in RStudio's file navigator menu. The third
video below documents how to save any changes (including new files and
folders) back to Renku.
