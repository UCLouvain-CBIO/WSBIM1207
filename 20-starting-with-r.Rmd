...
types you use while developing in R, RStudio will make typing easier
...
taste of how you can use R to analyze data for your own
...
or creating beautiful graphics. The best way to become proficient and
efficient at R, as with any other tool, is to use it to address your
actual research questions. As a beginner, it can feel daunting to have
to write a script from scratch, and given that many people make their
code available online, modifying existing code to suit your purpose
might make it easier for you to get started.
...
latter from CRAN (as show above), then we install `msmbstyle` directly
from the user `lgatto`'s github repository.

```{r ghinstall, eval=FALSE, purl=FALSE}
install.packages("devtools")
library("devtools")
install_github("lgatto/msmbstyle")
```
...
Recall that you can use the `typeof()` function to find the type of
your atomic vector.
...
- Check their type. Depending how they were created, they can be
  integers or doubles.
- Take the sum of each vector and verify they are identical.
...
- Create a vector named x containing the numbers 20 to 2. Retrieve
  elements that are strictly larger than 5 and smaller or equal than
  15.
