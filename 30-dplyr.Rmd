...
no `NA`s and all values are less than 30.
...
`tibble` of `r nrow(surveys)` observations is split into
`r length(unique(surveys$sex))` groups based on the `sex` variable.
...
In the spreadsheet lesson (chapter \@ref(sec:dataorg)), we discussed how
to structure our data leading to the four rules defining a tidy
dataset:
...
between, the plots. The key point here is that we are still following
a tidy data structure, but we have **reshaped** the data according to
the observations of interest: average species weight per plot instead
of recordings per date.
...
`spread()` and `gather()`. Recently, the new `pivot_longer()` and
`pivot_wider()` functions [have been
introduced](https://tidyr.tidyverse.org/dev/articles/pivot.html). They
come as a replacement for `gather()` and `spread()`, enabling more
advanced uses. The latter will remain available for the foreseeable
future.


### Spreading the data into a wide format

`spread()` and `pivot_wide` take three principal arguments:

1. the data
2. the `key` for `spread()` or `names_from` for `pivot_wider()` column
   variable whose values will become new column names.
3. the `value` for `spread()` or `values_from` for `pivot_wider()`
   column variable whose values will fill the new column variables.

Further arguments include `fill` which, if set, fills in missing
values with the value provided.

Let's use `spread()` (and `pivot_wider()` below) to transform surveys
to find the mean weight of each species in each plot over the entire
survey period. We use `filter()`, `group_by()` and `summarise()` to
filter our observations and variables of interest, and create a new
variable for the `mean_weight`. We use the pipe as before too.
...
`spread()` to key on `genus` with values from `mean_weight` this
becomes 24 observations of 11 variables, one row for each plot. We
again use pipes:
...
We could now plot comparisons between the weight of species in
different plots, although we may wish to fill in the missing values
first.
...
The equivalent call using the new `pivot_wider()` function is
...
To fill in the missing *mean_weigth* values with a 0, one would use
...
### Gathering data into a long format
...
`gather()` and `pivot_longer()` takes four principal arguments:

1. the data
2. the `key` for `gather()` or `names_to` for `pivot_longer()`
   column variable we wish to create from column names.
3. the `values` for `gather()` or `values_to` for `pivot_longer()`
   column variable we wish to create and fill with values associated
   with the key.
...
Using the `pivot_longer()` function, we would write
...
output as previously, we can arrange the data by `plot_id`:
...
type of problem. First, use `gather()` to create a dataset where we
have a key column called `measurement` and a `value` column that takes
on the value of either `hindfoot_length` or `weight`.
...
each `year` for each different `plot_type`. Then `spread()` them into
...
write_csv(tab, file = "data_output/tab.csv")
## or
write.csv(tab, file = "data_output/tab.csv")
```
...
clinical_mini %>%
  ...
  group_by(gender, smoking_history) %>%
	summarize(n = n()) %>%
  tidyr::spread(key = smoking_history, value = n)
```
...
Using `gather`, convert the data into a *long* table with a column
`interro` informing which test that line refers to and a column
`result` with the student's mark.
...
gather(x, key = interro, value = result,
