...
`pivot_wider()`, gathering new columns from the `genus` variable and
new values from `mean_weight`, this becomes 24 observations of 11
...
`values_fill` could also be a named list if you want to apply
different values to different value columns. To fill in the missing
*mean_weigth* values with a 0, one would use
...
Note that now the `NA` genera are included in the re-gathered
...
To re-arrange the data, we simply pipe le long data to `arrange()`.
