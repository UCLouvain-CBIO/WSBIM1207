...
```{r vis_setup, echo=FALSE, purl=FALSE}
...
```{r first-ggplot, purl=FALSE, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length)) +
  geom_point()
```
...
```{r adding-transparency, purl=FALSE, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length)) +
...
```{r adding-colors, purl=FALSE, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length)) +
...
```{r color-by-species1, purl=FALSE, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length)) +
...
```{r color-by-species2, purl=FALSE, cache=TRUE}
...
```{r color-by-species3, purl=FALSE, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length, color = species_id)) +
...
```{r boxplot, purl=FALSE, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = species_id, y = weight)) +
    geom_boxplot()
```
...
```{r boxplot-with-points, purl=FALSE, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = species_id, y = weight)) +
...
```{r facet-by-species-and-sex, purl=FALSE, cache=TRUE}
...
```{r facet-by-species-and-sex-white-bg, purl=FALSE, cache=TRUE}
 ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
     geom_line() +
     facet_wrap(~ species_id) +
     theme_bw() +
...
```{r number-species-year-with-right-labels, purl=FALSE, cache=TRUE}
ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
    facet_wrap(~ species_id) +
...
```{r number-species-year-with-right-labels-xfont-size, purl=FALSE, cache=TRUE}
ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
    facet_wrap(~ species_id) +
    labs(title = "Observed species in time",
...
```{r number-species-year-with-theme, purl=FALSE, cache=TRUE}
ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
    facet_wrap(~ species_id) +
    labs(title = "Observed species in time",
...
```{r number-species-year-with-right-labels-xfont-orientation, purl=FALSE, cache=TRUE}
...
*tidy data* and *tidy tools* pipeline. There are other visualiation
packages in R that shoudn't be ignored.

### base graphics {-}

The default graphics system that comes with R, often called *base R
graphics* is simple and fast. It is based on the *painters model*,
where different output are direcly overlaid on top of each other (see
figure \@ref(fig:paintermodel)). This is a fundamental difference with
`ggplot2` (and with `lattice`, described below), that returns
dedicated objects, that are rendered on screen or in a file, and that
can even be updated.
...
adapt their behavour based on the class of their input. This is again
...
par(mfrow = c(2, 2))
boxplot(rnorm(100), main = "Boxplot of rnorm(100)")
boxplot(matrix(rnorm(100), ncol = 10), main = "Boxplot of matrix(rnorm(100), ncol = 10)")
hist(rnorm(100))
...
complex (for example to produce facets), can become lengthly and
cumbersome.
...
facetting. `lattice` however isn't based on the grammar of graphics
