...
will be [freely available onlin](https://ggplot2-book.org/). The
`ggplot2` webpage
...
vector. Here is an example where we color with **`species_id`**:
...
properties (axies limits, ticks, ...) to facilitate their direct
comparison. We will use it to make a time series plot for each
species:
...
arrange figures side by side and the `/` to stack them on top of each
other.
...
visualise the results for each test and male/female students.
