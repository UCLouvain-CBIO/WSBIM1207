...
```{r adding-transparency, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length)) +
...
```{r color-by-species1, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length)) +
...
```{r color-by-species2, cache=TRUE}
...
```{r color-by-species3, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = weight, y = hindfoot_length, color = species_id)) +
...
```{r boxplot-with-points, cache=TRUE}
ggplot(data = surveys_complete, mapping = aes(x = species_id, y = weight)) +
...
axis to better distribute the observations in the space of the
...
```{r facet-by-species-and-sex, cache=TRUE}
...
```{r facet-by-species-and-sex-white-bg, cache=TRUE}
 ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
     geom_line() +
     facet_wrap(~ species_id) +
     theme_bw() +
...
```{r number-species-year-with-right-labels, cache=TRUE}
ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
    facet_wrap(~ species_id) +
...
```{r number-species-year-with-right-labels-xfont-size, cache=TRUE}
ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
    facet_wrap(~ species_id) +
    labs(title = "Observed species in time",
...
```{r number-species-year-with-theme, cache=TRUE}
ggplot(data = yearly_sex_counts, mapping = aes(x = year, y = n, color = sex)) +
    geom_line() +
    facet_wrap(~ species_id) +
    labs(title = "Observed species in time",
...
```{r number-species-year-with-right-labels-xfont-orientation, cache=TRUE}
...
```{r}
spp_count_plot <- spp_count_plot + theme(legend.position = "none")
library("patchwork")
spp_weight_boxplot + spp_count_plot
```

```{r}
spp_weight_boxplot + spp_count_plot + plot_layout(ncol = 1)
```
...
x <- gather(x, key = interro, value = result,
            starts_with("interro"))

ggplot(x, aes(x = gender, y = result)) +
    geom_boxplot() + facet_grid(. ~ interro)
```
