...
measured by the same operator in triplicate.

```{r, echo = FALSE}
expd <- data.frame(sample = paste0(rep(LETTERS[1:3], each = 3), 1:3),
                   operator = "Kevin", date = '2019-03-02',
                   group = rep(c("CTRL", "DRUG", "DRUG"), each = 3),
                   concentration = rep(c(0, 1, 5), each = 3),
...
variability in our measurements and, eventually identify changes that
related to the conditions of interest (i.e. presence of absence of the
drug).

```{r, echo = FALSE, fig.cap = "Caption comes here."}
ge <- tibble(Group = rep(expd$group, each = 3),
             Concentration = factor(rep(expd$concentration, each = 3)),
             Sample = rep(expd$sample, each = 3),
             Gene = rep(c("A1CF", "BRCA1", "TP53"), each = 9),
             Expression = c(rnorm(9, 6, 2),
                            c(rnorm(3, 5, 3), rnorm(3, 10, 4), rnorm(3, 13, 5)),
                            c(rnorm(3, 11, 5), rnorm(3, 10, 4), rnorm(3, 6, 3))))
                      
       
ggplot(ge, aes(x = Sample, y = Expression, colour = Gene)) +
    geom_boxplot()
```
