...
[RStudio](https://posit.co/products/open-source/rstudio/) is a very
popular way to not only write your R scripts but also to interact with
the R software[^plainr]. To function correctly, RStudio needs R and
...
IDE](https://github.com/rstudio/cheatsheets/raw/main/rstudio-ide.pdf).
...
[Bioconductor](https://bioconductor.org/) project, that we will
discuss in section \@ref(sec-bioconductor). Below we first install
...
library("BiocManager")
install("UCLouvain-CBIO/rWSBIM1207")
```
