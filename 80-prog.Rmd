...
In this recapitulative exercise, we are going to analyse the student
tests A and B data using functionality from the `dplyr` package (seen
in chapter \ref@(sec:dplyr)). The student test files are available in
the `rWSBIM1207` package (the `interroA.csv` and `interroB.csv`
functions return their respective paths). 
