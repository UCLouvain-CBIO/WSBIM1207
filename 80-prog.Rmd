...
1. Using the `interroA.csv()` and `interroB.csv()` functions from the
...
`r msmbstyle::question_begin()`
2. Iterate over the files and read each into an element of a
...
for (i in seq_along(fls))
    l[[i]] <- read.csv(fls[i], stringsAsFactors = FALSE)
