...
`x` and `y` and, depending whether the `x > y` or `x <= y`, generates 
the permutation `sample(x, y)` in the first case or draws a sample
from `rnorm(1000, x, y)` in the second case.
...
- Read multiple files into one dataframe (prepare data). Note: this
  will need to be demonstrated in class for many files, as the simple
  approach above won't scale.
